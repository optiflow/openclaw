...
The default Dockerfile now uses a multi-stage build:

- **Builder stage** (`node:22-bookworm`): installs full dependencies, runs
  `pnpm build`, and runs `pnpm ui:build`.
- **Runtime stage** (`node:22-bookworm-slim`): copies only runtime artifacts
  and production dependencies.

This keeps rebuilds cache-friendly while shrinking the final runtime image.
